...
> Pre-Gate-A المفتوحة في §1 **حُسِمت**: PG-1 (مسار `docs/`) مُنفَّذ (مجلّد `docs/` قائم)؛ PG-2 (رفع DOCUMENTATION-ONLY) تمّ عبر تفعيل Gate A وبناء حزم الأساس؛ المستودع وسياسة الفروع/الـ PR/CI قائمة. Gate A وما تلاه (Stages 2–10) **مُنجَزة كأساس read-only/advisory**. الخطة الكاملة المتبقّية إلى 100% (الأطوار A–E) في `BUILD-STATUS-AND-ROADMAP.md`. النصّ أدناه سجلّ تاريخي؛ عند أي تعارض تُغلَّب الوثيقة الحيّة + `docs/00`–`12`.
