...
> **آخر تحديث:** 2026-06-10 · **الحالة على main:** `e41fc00` · full suite **1642/1642** · mechanism guard `sources=107 allowlist=1 violations=0` · SSOT drift baseline ثابت · `can_send:true`/`can_broadcast:true` غائبان repo-wide · ALLOWLIST = `Object.freeze(['packages/isolated-signer-runtime/src/'])` · **الطور A مكتمل + بوّابته PASS** (`reports/E2-PHASE-A-GATE-...`) · **الطور B جارٍ (Stage 14 ✅ · Stage 15 التالي)**.
...
- **Stage 15 — Calibration & Backtest Harness — ⏭️ التالي:** `CalibrationStore` + معايرة point-in-time/survivorship-free + `candidate_paper_real_divergence` + تغذية بوّابات EV (`net_expectancy`/`profit_factor`/LCB/`exit_success`) + replay/backtest datasets. **بلا future leakage · المحافظ المنقرضة ضمن العيّنة.**
