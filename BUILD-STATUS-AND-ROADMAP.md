...
> **آخر تحديث:** 2026-06-10 · **الحالة على main:** `1eff85d` · full suite **1579/1579** · mechanism guard `sources=103 allowlist=1 violations=0` · SSOT drift baseline ثابت · `can_send:true`/`can_broadcast:true` غائبان repo-wide · ALLOWLIST = `Object.freeze(['packages/isolated-signer-runtime/src/'])`.
...
- **Stage 13 — End-to-End Decision-Trace Orchestrator (read-only) — ⏭️ التالي:** تركيب المراحل 4–12 في **Decision Trace** واحد deterministic + full-pipeline health/status read-model، يُظهر سبب القبول/الرفض لكل قرار عبر الأنبوب. لا تنفيذ. **Phase-A Gate:** مراجعة تكامل + regression كامل + توقيع حوكمي/أمني.
