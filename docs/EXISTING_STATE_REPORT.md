...
## 5. قبل/بعد (تُحدَّث في نهاية الجولة)

- **قبل:** `main` @ `59801c7` · 1854/1854 · حارسان أخضران · 3 وثائق تشغيلية مفقودة (هذا التقرير + الدليل + المقترحات).
- **بعد (هدف هذه الجولة):** نفس الكود (لا عيب كود لإصلاحه) + 3 وثائق تشغيلية مُنشأة ومدفوعة على فرع `release-gate/local-live/2026-06-11` + بصمة benchmark موثّقة. لا تغيير في `packages/` أو `docs/00`–`12` أو `tools/` أو ALLOWLIST؛ المجموعة تبقى 1854/1854.
