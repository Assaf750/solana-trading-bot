...
const PRESETS = [
  ...
  },
  {
    ...
    mode: 'warning_only',
    copy: { take_profit_pct: 150, stop_loss_pct: 35, trailing_stop_pct: 40, tp1_pct: 60, tp1_sell_pct: 33, breakeven_after_tp1: false, max_entry_slippage_vs_leader: 4, min_mirror_sell_pct: 30, max_entry_drift_pct: 8, drift_action: 'shrink', exit_on_leader_sell: false, auto_pause_after_losses: 5 },
...
export default function SettingsSafety() {
  ...
  async function loadConfig() {
    ...
    if (r.ok) {
      ...
      setCopyDef({
        ...
        tp1_pct: g(d.copy_defaults, 'tp1_pct'), tp1_sell_pct: g(d.copy_defaults, 'tp1_sell_pct'),
        breakeven_after_tp1: Boolean(d.copy_defaults?.breakeven_after_tp1),
      });
      setExec({
  ...
  async function save() {
    ...
    const patch = {
      ...
      copy_defaults: {
        ...
        tp1_pct: numOrNull(copyDef.tp1_pct), tp1_sell_pct: numOrNull(copyDef.tp1_sell_pct),
        breakeven_after_tp1: !!copyDef.breakeven_after_tp1,
      },
      execution: {
  ...
  const copyFields = [
    ...
    ['tp1_pct', ar ? 'جني جزئي TP1 %' : 'Partial TP1 %'],
    ['tp1_sell_pct', ar ? 'بيع عند TP1 %' : 'Sell at TP1 %'],
        ...
        <>
          ...
          <Card title={ar ? 'افتراضات النسخ العامة' : 'Global copy defaults'} sub={ar ? 'تُستخدم ما لم تُستبدل لكل محفظة.' : 'Used unless overridden per-wallet.'}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--s-2)' }}>
              ...
              <label className="row" style={{ gap: 8, alignSelf: 'end' }}>
                <input type="checkbox" checked={!!copyDef.breakeven_after_tp1} onChange={(e) => setCopyDef({ ...copyDef, breakeven_after_tp1: e.target.checked })} />
                <span className="fs-xs">{ar ? 'تعادل بعد TP1 (حماية الموونباغ)' : 'Break-even after TP1 (moonbag)'}</span>
              </label>
            </div>
          </Card>
