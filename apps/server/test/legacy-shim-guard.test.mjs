...
// ---------- RISK_BACKEND — pure gate; legacy must be byte-identical to package (parity before removal) ----------
const FULL_HR = {
...
test('RISK_BACKEND: legacy === package; default + unknown resolve to package (allow + reject paths)', () => {
  for (const args of [baseArgs(), { ...baseArgs(), killBlocked: true }]) {
    const legacy = withEnv('RISK_BACKEND', 'legacy', () => checkEntryGates(args));
    const pkg = withEnv('RISK_BACKEND', 'package', () => checkEntryGates(args));
    const def = withEnv('RISK_BACKEND', undefined, () => checkEntryGates(args));
    const unknown = withEnv('RISK_BACKEND', 'garbage', () => checkEntryGates(args));
    assert.deepEqual(legacy, pkg, 'legacy is byte-identical to package');
    assert.deepEqual(def, pkg, 'default (unset) = package');
    assert.deepEqual(unknown, pkg, 'unknown value resolves to package, never legacy');
  }
  assert.equal(withEnv('RISK_BACKEND', undefined, () => checkEntryGates(baseArgs())).allowed, true);
  assert.equal(withEnv('RISK_BACKEND', undefined, () => checkEntryGates({ ...baseArgs(), killBlocked: true })).allowed, false);
});
