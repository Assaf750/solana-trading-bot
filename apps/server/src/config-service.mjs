...
const DEFAULTS = {
  ...
  market_filters: {
    // optional entry quality filters. null = off. They SKIP when data can't be read (NOT the
    // fail-closed anti-rug screen). FDV = fully-diluted valuation band; min_holders via Helius DAS.
    min_fdv_usd: null,
    max_fdv_usd: null,
    min_holders: null,
  },
...
const NUMERIC_BOUNDS = {
  ...
  min_fdv_usd: [1, 1e15], max_fdv_usd: [1, 1e15], min_holders: [1, 1e7],
...
export function validateConfigPatch(patch) {
  ...
  const sections = {
    ...
    market_filters: ['min_fdv_usd', 'max_fdv_usd', 'min_holders'],
